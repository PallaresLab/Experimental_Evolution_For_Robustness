...
```{r 7 -> Flight analysis - changes associated with diet, echo=FALSE}
...
# 6.1 - To integrate flight analysis with the dataset, we need to import all assigned ID values ordered by Y-axis as well as individual flight values in cm-s. We first merge these two datasets and then overlap that dataframe with the wing coordinates based on the 'ID' colomn
...
# 6.3 - Once we checked if the shape of avareage wing per individual correlated with flight, we need to further test if symetry also correlates with flight. To go ahead, we first subset data that always have two wings per individual
...
#step 2 - check if crossveinless mark (landmark 10) has a strong impact on overall shape of PCA, and variance in wings.

```

```{r 8 -> Object summary, include=FALSE}
