...
colnmaes = c("id","X01","Y01","X02","Y02","X03","Y03","X04","Y04","X05","Y05","X06","Y06","X07","Y07","X08","Y08","X09","Y09","X10","Y10","X11","Y11","X12","Y12","X13","Y13","X14","Y14","X15","Y15")
...
#complete$Replica = #make sure you define this before merging
  
complete$symetry = ifelse(str_detect(complete$id, "R"), print("R"), print("L"))
complete$id = paste(complete$id,complete$Replica, complete$condition, complete$symetry, sep = "_")
...
myGPA$Replica = NEX_wings$Replica
...
sizes_per_group = as.data.frame(myGPA$Csize, myGPA$Cond_full,myGPA$Replica)
...
symetry_per_group = as.data.frame(bs$signed.AI, bs$condition, bs$Replica) #check if bs has replica colomns
#symetry_per_group$Replica = bs$Replica
...
#let's make this plot not only based on conditions but also based on replicas
ggbetweenstats(
  data  = symetry_per_group,
  x     = Replica,
  y     = AI,
...
dim(NEX_wings_sm)
dim(NEX_wings_as)
NEX_wings_as_average <- aggregate(NEX_wings_sm, by=NEX_wings_sm["pair"], mean ) #generate an average wing when two wings are present 

#this creates a duplicated object entry, need a better way to arrange this
...
NEX_wings_as_average_size_asymetrty <- aggregate(NEX_wings_sm$size, by=NEX_wings_sm["pair"], minus )
NEX_wings_as_average$size_difference = NEX_wings_as_average_size_asymetrty$size

#Nex_wings_AV =  NEX_wings_as_average (average of two wings when two wings are present) + NEX_wings_as (one wing if only one is present)
NEX_wings_as$size_difference = NA #this colomn needs to be present for other analysis, if flies have one wing, there is no size difference in wings that can be assigned, so NA values are added ahead of merging two dataframes.
Nex_wings_AV = rbind(NEX_wings_as_average,NEX_wings_as)

# this needs to be assigned separately because main obejcts has a different order of individuals.
...
ggbetweenstats(data  = Nex_wings_AV,x     = condition, y     = logsize,
  title = "Wing size distribution per group", pairwise.display = "significant", centrality.plotting = F, p.adjust.method = "bonferroni",xlab = "Groups",results.subtitle = FALSE, ylab = "Log 10 -Wing Centrod Size", palette = "Accent")
#make the same plot but for different replicas
ggbetweenstats(data  = Nex_wings_AV,x     = Replica, y     = logsize,
  title = "Wing size distribution per group", pairwise.display = "significant", centrality.plotting = F, p.adjust.method = "bonferroni",xlab = "Groups",results.subtitle = FALSE, ylab = "Log 10 -Wing Centrod Size", palette = "Accent")

# 7.2 - To integrate flight analysis with the dataset, we need to import all assigned ID values ordered by Y-axis as well as individual flight values in cm-s. We first merge these two datasets and then overlap that dataframe with the wing coordinates based on the 'ID' colomn
...
Go_females_0523_flight_complete <-read.csv("Go_females_0523_flight_complete.csv", row.names = NULL)
#I also concatenated each order_id into file where each column 
Go_female_0523_orderID_complete <-read.csv("Go_female_0523_orderID_complete.csv")
#wing data - there are several versions depending on amount of information/wings included. Nex_wings_AV provides the most useful starting point.
Go_female_0523_wings_complete = Nex_wings_AV
...
# 7.3 - Once we checked if the shape of avareage wing per individual correlated with flight, we need to further test if symmetry also correlates with flight. To go ahead, we first subset data that always have two wings per individual
#Here we need to transition to AI index which is exclusively assigned to flies that have both wings present, we extract it from bilat.symetry (bs) object.
AI_NEX = as.data.frame(bs$signed.AI)
...
G0_ID_annotated_crossveinless <-read_csv("~/Desktop/GitHub/2023_Experimental_Evolution_For_Robustness/code/data/G0_ID_annotated_crossveinless.csv")
...
#this file needs to be transposed + ids made in a consistent manner with the rest of the objects. Currently it is = flight_id + Replica + condition + symmetry (L/R) and separated by an underscore.

```


```{r 9 -> Object integration heatmap, include=FALSE}
#9.1 This plot should show integration of all traits in a form of a heatmap. One limiting factor in this heatmap is size_difference colpomn as well as asymetry index (ai) colomn because some flies do not have those values. I will those create a heatmap that has all data and a supplementary heatmap without ai index to include the rest of the flies.
...
# step 1 - use AI_NEX_merges_na as the starting point because it has all data merged and NA removed. 
#to test the functions I used a dummy file with all colomns/conditions specified, this can be accessed if necessary
#dummy_heatmap <- readRDS("~/Desktop/GitHub/2023_Experimental_Evolution_For_Robustness/code/supplementary/dummy.RDS")

#This data frame provides: flight data, coords, ai_index, sizes, and size differences
#This format contains everything needed (landmarks, size, flight, etc). Need to be adjusted first.
...
wing_variation = ShapeDist(shapes = AI_NEX_merges_na[, 11: 40], reference = mshape(arrayspecs(dummy_heatmap[, 11: 40], 15, 2)))
...
size_robustness = abs(AI_NEX_merges_na$size - median(AI_NEX_merges_na$size))
#flight MAD
flight_robustness = abs(AI_NEX_merges_na$Y - median(AI_NEX_merges_na$Y)) #One problem of this approach above is that we need to normalize mad scores for each replica?
ai_robustness = abs(AI_NEX_merges_na$ai - median(AI_NEX_merges_na$ai))
...
size_asymmetry_robustness = abs(AI_NEX_merges_na$LRsize_difference - median(AI_NEX_merges_na$LRsize_difference))
...
#classifier = as.data.frame(AI_NEX_merges_na$condition,AI_NEX_merges_na$pair)
...
plotPCA.df = data.frame(PC1 = PCA$x[,1], PC2 = PCA$x[,2], PC3 = PCA$x[,3], PC4 = PCA$x[,4])
                        sample = AI_NEX_merges_na$id,  #change meta data here
                       condition = AI_NEX_merges_na$condition, batch = AI_NEX_merges_na$Replic) #there is a chance to make corrections for batch or assays here

qplot(PC1, PC2, data = plotPCA.df, color = as.factor(AI_NEX_merges_na$condition), main = "PC1 vs PC2 of robustness scores",
      size = I(6))+ labs(x = paste0("PC1, VarExp:", round(percentVar[1],4)),  y = paste0("PC2, VarExp:", round(percentVar[2],4))) + scale_color_manual(values = colorRampPalette(brewer.pal(n = 2, name = "RdBu"))(4))+ theme_classic() + theme(aspect.ratio=1)


qplot(PC1, PC2, data = plotPCA.df, color = as.factor(AI_NEX_merges_na$Replica), main = "PC1 vs PC2 of robustness scores",
      size = I(6))+ labs(x = paste0("PC1, VarExp:", round(percentVar[1],4)),  y = paste0("PC2, VarExp:", round(percentVar[2],4))) + scale_color_manual(values = colorRampPalette(brewer.pal(n = 12, name = "RdBu"))(4))+ theme_classic() + theme(aspect.ratio=1)
